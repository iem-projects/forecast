...
# </video>
#
# Modifications HHR: remove requirement to have URLs start with http(s). add loop parameter
...
module Jekyll
  ...
  class VideoTag < Liquid::Tag
    ...
    def initialize(tag_name, markup, tokens)
      ...
      if markup =~ /(\D\S+)(\s+(\D\S+))?(\s+(\D\S+))?(\s+(\d+)\s(\d+))?(\s+(\S+))?(\s+(\S+))?/i
        @video  = [$1, $3, $5].compact
        @width  = $7
        @height = $8
        @poster = $10
    ...
    def render(context)
      ...
      if @video.size > 0
        video = "<video width='#{@width}' height='#{@height}' preload='none' #{@loop} controls poster='#{@poster}'>"
        @video.each do |v|
          t = v.match(/([^\.]+)$/)[1]
